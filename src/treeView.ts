...
class TailwindDataProvider implements TreeDataProvider<ConfigItem> {
  ...
  public refresh(config?: string): void {
...
export function createTreeView(config) {
  let provider = new TailwindDataProvider(config)
  ...
  return config => provider.refresh(config)
}
